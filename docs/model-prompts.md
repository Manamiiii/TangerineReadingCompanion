...
| `reading-evidence-v1` | 读者查找当前内容依据 | 当前问题、最多 6000 字当前段落及最多 20 条已解锁背景短注释 | 最多 3 条 sourceId 与逐字摘录，每条最多 800 字 | 仅会话显示；不接受自由生成的 answer |
...
应用丢弃索引外的 id；候选名称还必须与目标实体的标准名、原文名或已有别名规范化后相同，否则解除配对。只有通过此检查才使用资料的类型和地点性质。模型名称还必须在本机通过当前段落精确出现检查，英文要求完整词边界；模型自行补出的名称不会展示。候选仍需读者确认才进入已遇到记录；确认后保留段落中的实际译名，同时保存资料实体 id，使不同译名能够复用同一份背景、地图和去重记录。读者编辑候选名称或类型时配对自动解除，避免修改后的文本沿用旧关系。
...
你为 Tangerine Reading Companion 准备内置书资料包的研究候选。
...
`reading-evidence-v1` 将模型限制为依据选择器：只从当前原文或已解锁背景中选择连续原句，不自由生成作品解释。应用逐项校验 sourceId、摘录长度和原文包含关系，再用本机来源标签展示；未知来源、改写或编造的摘录整次拒绝。没有输入依据时不调用模型。当前问题仍在请求前拦截明显后续导向；关键词规则仅作为额外限制，不作为任意生成内容的无剧透证明。依据相关性仍需读者判断；本机校验保证展示文字来自所提供的范围，不保证模型选段一定回答了问题。后续如恢复自由解释，需要另行建立可核验的权限与证据策略。

模型请求的 45 秒超时覆盖网络请求及响应体读取。切书、章节变化、清空或替换输入时清空临时模型缓存并取消在途请求；组件和地图目标变化另外使用操作令牌和 AbortSignal 阻止迟到结果。模型接口错误不展示供应商返回的任意原文，以免回显密钥或段落。个人名称词典仍是明确允许的持久化辅助资料，不代表正式事实。

地图检索词 `place-query-v1` 与书目整理 `book-metadata-v4` 的消息模板也集中在 `promptCatalog.js`；个人名称准备上限由同一常量约束提示词和输出校验。当前原文超过 6000 字时依据查找明确拒绝请求并提示缩短，不静默截断。
