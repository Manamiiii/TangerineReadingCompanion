...
日常进度、名称、备注与地图修改在同一个写事务内读取最新记录并执行同步领域操作。界面提交针对对象的操作，不提交由旧界面快照计算的整组记录，避免快速操作或双标签页写入相互覆盖。个人词典补充与删除也在事务内操作，已删除的书籍不会被迟到的准备任务重新创建。备份导入仍为同 key 整条覆盖，不改为字段合并，不修改 Dexie schema。
